...
async function inyectarInterfaz(datos) {
    ...
    estilo.textContent = `
        * { box-sizing: border-box; }
        #cl-micro-banner { position: fixed; top: 20px; right: 20px; z-index: 2147483647; background: #1e293b; color: white; padding: 12px 18px; border-radius: 12px; font-family: 'Segoe UI', sans-serif; box-shadow: 0 10px 25px rgba(0,0,0,0.5); display: flex; align-items: center; gap: 15px; border: 1px solid #334155; }
        .cl-brand-logo { width: 28px; height: 28px; }
        #cl-modal-overlay { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(15, 23, 42, 0.9); display: none; justify-content: center; align-items: center; z-index: 2147483647; font-family: 'Segoe UI', sans-serif; }
        #cl-modal-card { background: #1e293b; width: 500px; border-radius: 16px; color: white; overflow: hidden; box-shadow: 0 25px 50px rgba(0,0,0,0.5); border: 1px solid #334155; }
        .cl-card-alt { background: #0f172a; padding: 18px; border-radius: 12px; margin-bottom: 15px; border: 1px solid #334155; }
    ...
    while (doc.body.firstChild) {
        contenedor.appendChild(doc.body.firstChild);
    }
